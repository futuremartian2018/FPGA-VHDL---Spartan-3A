...
;------------------------------------------------------------------
;Frequency counter. LSB from 74HC590 read at P0, TL0, TH0 and TF0OVF.
;OUT:	32 Bit result in R7:R6:R5:R4
;------------------------------------------------------------------
FRQCOUNT:	CLR	P1.1				;MRC, Clear 74HC590 counter
		SETB	P1.1
		MOV	TL0,#00h
		MOV	TH0,#00h
		MOV	TF0OVF,#00h
		...
		CLR	ACC.5				;TF0
		MOV	TCON,A
		CLR	P1.0				;Enable 74HC590 Count
		ACALL	WAITASEC
		SETB	P1.0				;Disable 74HC590 Count
		MOV	R4,P0
		MOV	R5,TL0
		MOV	R6,TH0
		MOV	R7,TF0OVF
